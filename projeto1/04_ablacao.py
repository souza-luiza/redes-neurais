...
# ============================================================
# FIGURA 2 - COMPARAÇÃO DAS CURVAS DE VALIDAÇÃO
# ============================================================
...
# Baseline
plt.plot(
    range(1, len(baseline["history"]["val_loss"]) + 1),
...
# Momentum
plt.plot(
    range(1, len(momentum_best["history"]["val_loss"]) + 1),
    momentum_best["history"]["val_loss"],
    label="Momentum ($\\beta=0.5$)"
)

# L2
plt.plot(
    range(1, len(l2_best["history"]["val_loss"]) + 1),
    l2_best["history"]["val_loss"],
    label="L2 ($\\lambda=10^{-5}$)"
)

# L1
plt.plot(
    range(1, len(l1_best["history"]["val_loss"]) + 1),
    l1_best["history"]["val_loss"],
    label="L1 ($\\lambda=10^{-3}$)"
)

# Dropout
plt.plot(
    range(1, len(dropout_best["history"]["val_loss"]) + 1),
    dropout_best["history"]["val_loss"],
    label="Dropout ($p=0.1$)"
)
...
plt.title(
    ...
    "Baseline vs. Modelos de Ablação"
)
...
plt.savefig(
    "Figura2_Comparacao_Validacao.png",
    dpi=300
)
